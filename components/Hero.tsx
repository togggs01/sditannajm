...
function CounterStat({ end, suffix = '+', label }: { end: number; suffix?: string; label: string }) {
  ...
  return (
    <div ref={ref} className="bg-white/10 backdrop-blur-sm rounded-lg sm:rounded-xl p-4 sm:p-6 border border-white/20 shadow-lg">
      <div className={`text-2xl sm:text-4xl font-bold text-[#f7e98e] mb-1 sm:mb-2 transition-opacity duration-500 drop-shadow-lg ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
        {count}{suffix}
      </div>
  ...
  return (
    <div className="relative text-white overflow-hidden pt-20">
      ...
      <div className="absolute inset-0">
        ...
        <div className={`absolute inset-0 ${
          hasImage && !imageError
            ? 'bg-gradient-to-br from-[#4a7c2a]/85 via-[#5a8f35]/75 to-[#4a7c2a]/85' 
      ...
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-0 left-0 w-96 h-96 bg-gradient-to-br from-[#f7e98e] to-[#f4d03f] rounded-full filter blur-3xl"></div>
        <div className="absolute bottom-0 right-0 w-96 h-96 bg-gradient-to-tl from-[#f4d03f] to-[#d4af37] rounded-full filter blur-3xl"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-gradient-to-r from-[#d4af37]/40 to-[#f7e98e]/40 rounded-full filter blur-3xl"></div>
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-16 sm:py-24 sm:px-6 lg:px-8 z-10">
        <div className="text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center px-4">
            ...
            <Link 
              href="/tentang" 
              className="w-full sm:w-auto group bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white font-bold py-3 sm:py-4 px-8 sm:px-10 rounded-xl text-base sm:text-lg transition-all border-2 border-white/30 hover:border-[#f7e98e]/50 shadow-lg hover:shadow-xl flex items-center justify-center"
            >
